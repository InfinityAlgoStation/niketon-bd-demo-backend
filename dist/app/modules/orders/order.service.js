...
const createOrderToDB = payload =>
  __awaiter(void 0, void 0, void 0, function* () {
    // check buyer
    const buyerCurrentBudget = yield users_model_1.User.findById(
      payload.buyer,
      { budget: 1 }
    );
    ...
    // check cow
    const cowInfo = yield cow_model_1.Cow.findById(payload.cow, {
      price: 1,
      seller: 1,
    });
    if (!cowInfo) {
    ...
    // check seller
    const sellerIncome = yield users_model_1.User.findById(
      cowInfo === null || cowInfo === void 0 ? void 0 : cowInfo.seller,
      { income: 1 }
    );
    if (!sellerIncome) {
      throw new ApiErrors_1.default(
    ...
    if (buyerCurrentBudget && cowInfo) {
      hasEnoughMoney =
        Number(cowInfo.price) <= Number(buyerCurrentBudget.budget);
      updateBuyerBudget =
    ...
    if (!hasEnoughMoney) {
      ...
      try {
        ...
        const updateCow = yield cow_model_1.Cow.findByIdAndUpdate(
          payload.cow,
          { label: 'sold out' },
          { session }
        );
        ...
        // update budget to db
        const updateBudget = yield users_model_1.User.findByIdAndUpdate(
          payload.buyer,
          {
        ...
        // update income to db
        const updateIncome = yield users_model_1.User.findByIdAndUpdate(
          cowInfo === null || cowInfo === void 0 ? void 0 : cowInfo.seller,
          {
